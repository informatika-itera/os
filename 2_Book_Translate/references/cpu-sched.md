...
Let’s do a quick example together. Imagine three jobs arrive in the system, A, B, and C, at roughly the same time (). Because FIFO has to put some job first, let’s assume that while they all arrived simultaneously, A arrived just a hair before B which arrived just a hair before C. Assume also that each job runs for 10 seconds. What will the average turnaround time be for these jobs?

**Figure 7.1: FIFO Simple Example**
*(Note: Figure shows A running 0-10, B running 10-20, C running 20-30)*

From Figure 7.1, you can see that A finished at 10, B at 20, and C at 30. Thus, the average turnaround time for the three jobs is simply . Computing turnaround time is as easy as that.
...
*(Note: Figure shows A running 0-100, B running 100-110, C running 110-120)*
...
*(Note: Figure shows B running 0-10, C running 10-20, A running 20-120)*
...
Here we can illustrate the problem again with an example. This time, assume A arrives at  and needs to run for 100 seconds, whereas B and C arrive at  and each need to run for 10 seconds. With pure SJF, we’d get the schedule seen in Figure 7.4.
...
*(Note: A starts at 0. B, C arrive at 10. A continues until 100. B runs 100-110. C runs 110-120)*
...
*(Note: A runs 0-10. B, C arrive. B runs 10-20. C runs 20-30. A resumes and runs 30-120)*
...
*(Note: A runs 0-5, B runs 5-10, C runs 10-15)*

**Figure 7.7: Round Robin (Good For Response Time)**
*(Note: A, B, C, A, B, C... cycling every 1 unit)*
...
*(Note: Figure shows CPU running A, then waiting while Disk runs A's I/O, then running A again... inefficient)*

**Figure 7.9: Overlap Allows Better Use Of Resources**
*(Note: Figure shows CPU running B while Disk is processing A's I/O)*
...
[W15] "You can’t have your cake and eat it" by Authors: Unknown.. Wikipedia (as of December 2015). `http://en.wikipedia.org/wiki/You.can't_have_your_cake_and_eat_it`. The best part of this page is reading all the similar idioms from other languages. In Tamil, you can’t "have both the moustache and drink the soup."
