...
%================================================
% BAGIAN 4: LOCK PITFALLS — PREVIEW DEADLOCK
%================================================
\section{Jebakan Lock dan Preview Deadlock}

\begin{frame}[plain]
    \begin{center}
        \vspace{3cm}
        \Huge\textbf{Jebakan Lock}\\[0.4em]
        \Large\textit{Apa yang Bisa Salah — dan Minggu Depan}
    \end{center}
\end{frame}

%--- Slide 24: Lupa unlock ---
\begin{frame}[fragile,shrink=5]
    \frametitle{Jebakan 1: Lupa Membuka Kunci}
    \framesubtitle{Sudah dibahas di Section 1 — tapi ini yang paling sering terjadi}
    \footnotesize

    \begin{columns}[T]
        \column{0.50\textwidth}
        \begin{alertblock}{Contoh nyata — sulit dilihat}
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize]
pthread_mutex_lock(&lock);

if (data == NULL) {
    log_error("data kosong");
    return -1;   // BAHAYA: unlock terlewat
}
...
\end{lstlisting}
        ...
        \begin{block}{Apa yang terjadi?}
            \begin{itemize}
                \item Kalau \texttt{data == NULL}, fungsi \texttt{return} lebih dulu
                \item \texttt{unlock} tidak pernah dipanggil
                \item Lock tetap terkunci \tem{selamanya}
                \item Thread lain menunggu — \textbf{program macet}
            \end{itemize}
        \end{block}

        \begin{exampleblock}{Cara aman}
            Selalu pastikan setiap jalur keluar dari fungsi memanggil \texttt{unlock}.\\
            Atau gunakan RAII (di C++).
        \end{exampleblock}
    \end{columns}
\end{frame}

%--- Slide 25: Deadlock dua thread ---
\begin{frame}[shrink=5]
    \frametitle{Jebakan 2: Deadlock — Dua Thread Saling Tunggu}
    \framesubtitle{Ini lebih berbahaya — tidak ada yang bisa maju}
    \footnotesize
...
%--- Slide 26: Preview minggu depan ---
\begin{frame}[shrink=5]
    \frametitle{Minggu Depan: Deadlock Lebih Dalam}
    \framesubtitle{Hari ini baru melihat — minggu depan kita bedah tuntas}
    \small

    \begin{columns}[T]
        \column{0.48\textwidth}
        \begin{block}{Yang sudah kita tahu hari ini}
            \begin{itemize}
                \item Deadlock terjadi saat dua thread saling menunggu lock
                \item Lupa unlock bisa menyebabkan program macet
                \item Urutan lock yang berbeda = bahaya
            \end{itemize}
        \end{block}

        \column{0.48\textwidth}
        \begin{exampleblock}{Yang akan dibahas minggu depan}
            \begin{itemize}
                \item \textbf{Empat kondisi} yang harus ada agar deadlock terjadi
                \item \textbf{Resource allocation graph} — cara menggambar deadlock
                \item \textbf{Cara mencegah} deadlock sejak awal
                \item \textbf{Cara mendeteksi} deadlock yang sudah terjadi
            \end{itemize}
        \end{exampleblock}
    \end{columns}

    \vspace{0.5em}
    \begin{alertblock}{Pesan untuk minggu ini}
        \centering
        \footnotesize
        Selalu \textbf{lock} → kerja → \textbf{unlock} dalam urutan yang konsisten.\\
        Jangan pernah keluar dari fungsi tanpa melepas semua lock yang sudah diambil.
    \end{alertblock}
\end{frame}
